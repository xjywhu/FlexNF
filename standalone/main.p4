...
control IngressPipeImpl (inout parsed_headers_t    hdr,
    ...
    action tunnel_forward(port_num_t port_num) {
        standard_metadata.egress_spec = port_num;
    }

    table tunnel_forward_table {
        key = {
            hdr.tunnel.id: ternary;
        }
        actions = {
            tunnel_forward;
        }
        @name("tunnel_forward_table_counter")
        counters = direct_counter(CounterType.packets_and_bytes);
    }
    ...
    action embed_tag(bit<9> id) {
        hdr.tunnel.setValid();
        hdr.tunnel.id = id;
        hdr.ipv6.flow_label = 1;
        hdr.ethernet.ether_type = ETHERTYPE_TUNNEL;
    }
    ...
    action shift(){
        hdr.tunnel.id = hdr.tunnel.id >> 3;
    }

    table shift_tag_table{
        actions = {
            shift;
        }
        key = {
