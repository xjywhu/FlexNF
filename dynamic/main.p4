...
control big_flow_detector(inout parsed_headers_t hdr, inout local_metadata_t local_metadata, inout standard_metadata_t standard_metadata){
    ...
    action read_state(){
        hash(local_metadata.hash_val,
        HashAlgorithm.crc16,
        32w0,
        { hdr.ipv6.src_addr,
          hdr.ipv6.dst_addr},
        32w16);
        state.read(local_metadata.state, local_metadata.hash_val);
        count.read(local_metadata.count, local_metadata.hash_val);
    }
    ...
    action goto_small(){
        //update count number
        count.write(local_metadata.hash_val, local_metadata.count+8w1);
    }

    action goto_big(){
        //update the state to big
        state.write(local_metadata.hash_val, 1w1);
    }
...
control stateful_firewall(inout parsed_headers_t hdr, inout local_metadata_t local_metadata, inout standard_metadata_t standard_metadata) {

    register<bit<3>>(16) fw_reg;
    action drop() {
        mark_to_drop(standard_metadata);
    }
    
    action read_fw_state(){
        bit<32> tmp_state1;
        bit<32> tmp_state2;
        ...
        local_metadata.fw_hash_val = tmp_state1+tmp_state2;
        fw_reg.read(local_metadata.fw_state, local_metadata.fw_hash_val);
    }

    action state_advance(bit<2> c){
        //update the state
        local_metadata.fw_condition = c;
    }

    action branch_drop(){
        local_metadata.fw_condition = 0;
    }

    action pass(bit<3> next_state){
        //update the state
        fw_reg.write(local_metadata.fw_hash_val, next_state);
    }
    ...
    table state_table{
        ...
        size = 1;
        default_action = read_fw_state();
    }

    table condition_table{
        key = {
            local_metadata.fw_state: exact;
            hdr.tcp.ctrl: exact;
        }
        actions = {
            state_advance;
        ...
        size = 126;
        default_action = branch_drop;
    }

    table action_table{
        key = {
            local_metadata.fw_state: exact;
            local_metadata.fw_condition: exact;
        }
        actions = {
            drop;
            pass;
...
control IngressPipeImpl (inout parsed_headers_t    hdr,
    ...
    // --- forward_table (for unicast entries) --------------------------------
    ...
    action get_label(bit<6> label){
        local_metadata.label = label;
    }
    ...
    apply {
        ...
        // if(local_metadata.label&6w0b111111 != 0){
        //     if((local_metadata.label&6w1) != 0){
        //         nf0.apply(hdr, local_metadata, standard_metadata);
        //     }
        //     if((local_metadata.label&6w2) != 0){
        //         nf1.apply(hdr, local_metadata, standard_metadata);
        //     }
        //     if((local_metadata.label&6w4) != 0){
        //         nf2.apply(hdr, local_metadata, standard_metadata);
        //     }
        //     if((local_metadata.label&6w8) != 0){
        //         nf3.apply(hdr, local_metadata, standard_metadata);
        //     }
        //     if((local_metadata.label&6w16) != 0){
        //         nf4.apply(hdr, local_metadata, standard_metadata);
        //     }
        //     if((local_metadata.label&6w32) != 0){
        //         // resub_table.apply();
        //         resubmit(local_metadata);
        //     }
        // }
